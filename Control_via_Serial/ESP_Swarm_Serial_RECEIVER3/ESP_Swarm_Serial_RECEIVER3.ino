...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  ...
  waveformType = constrain(myData.x, 1, 4);  
  
  targetFreq[0] = myData.r;
  targetFreq[1] = myData.s;
  targetFreq[2] = myData.t;

  targetVolume[0] = (float)myData.u / 255.0 / NUM_OSC;
  targetVolume[1] = (float)myData.v / 255.0 / NUM_OSC;
  targetVolume[2] = (float)myData.w / 255.0 / NUM_OSC;

  targetVolNoise = (float)myData.y / 255.0;  
